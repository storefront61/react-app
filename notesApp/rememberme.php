...
if(!isset($_SESSION['user_id']) && !empty($_COOKIE['rememberme'])){
	//(another option)array_key_exists('user_id', $_SESSION);
	//f1 COOKIE: $c = $a . "," . bin2hex($b);
	//f2  hash('sha256', $a);
    ...
    // look for authentificator1 in the rememberme table 
        ...
        if(!$result){
        	...
        	echo 'Error: ' . mysqli_error() . ' in query';
        	exit;
        }
	    ...
	    //if authentificator2 does not match print error
		    if(!hash_equals($row['f2authentificator2'], $f2authentificator2)){//======left off on sat
